\def\strlit#1{\textsf{\scriptsize ``#1''}}
\def\cname#1{\textsf{#1}}
...
$exports$ & $:\fsep \mathcal P (O)$         & \mcom{exported ops}\\
$invokes$ & $:\fsep \mathcal P (O)$         & \mcom{invoked ops}\\
$stores$  & $:\fsep N \pfun \mathcal P (D)$ & \mcom{stored data}\\
$guards$   & $:\fsep O \pfun F$              & \mcom{guards on ops}\\
\multicolumn{3}{l}{\textbf{Operation} ($O$)} \\
$args$    & $:\fsep N \pfun \mathcal P (D)$ & \mcom{arguments}\\
\end{tabular}
\end{minipage}
\end{minipage}
}
...
\begin{tabular}{@{\quad}r@{\;}l}
  ...
  $incoming(c)$ & $= {\{ o \in c.exports \;|\; c.guards(o) \}}$ \\
...
\end{minipage}
\begin{tabular}{ll}
\multicolumn{2}{l}{\mcom{operations and behavior}}\\
...
\\
\begin{tabular}{ll}
$\cname{NYTimes}.guards(\cname{GetPage}) $  & $= f_1(o) \equiv o.args(\strlit{currCounter}) \leq \cname{LIMIT}$\\
$\cname{NYTimes}.guards(\cname{SendPage}) $ & $= f_2(o) \equiv \exists t \in \cname{GetPage} \;\;\bullet $\\
 & \bigsep $ t.args(\strlit{page}) = \cname{NYTimes}.stores(\strlit{articles})[t.args(\strlit{link})] \;\land $ \\
...
Figure~\ref{fig-nytimes-spec}, \cname{NYTimes} only accepts a request
for an article only if \cname{currCounter} provided with the request
...
along with the invoked operations. For example, \cname{NYTimes} may
invoke \cname{SendPage} on the client only if it has already received
and accepted a valid \cname{GetPage} message. In addition,
\cname{SendPage} must include, as arguments, an article that
