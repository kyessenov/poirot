\def\strlit#1{\textsf{\color{stringcolor} ``#1''}}
\def\cname#1{\textsf{#1}}
...
$guard$   & $:\fsep O \pfun F$              & \mcom{guards on operations}\\
...
\caption{A snippet of the New York Times paywall model. Assume that
  \txtcname{Map[K, V]} is a predefined data type that represents a map
  from \txtcname{K} to \txtcname{V}; $m[k]$ returns the value stored in
  map $m$ at key $k$.}
