...
of data $D$\fix{ that can be transmitted}{. Data can flow from one} to
another module \fix{by \textit{invoking} the latter's operations}{only
  by means of operation \textit{invocations}; this enables our system
  to accurately track data flows and automatically check standard
  ...
  paragraphs below, where it talks about passing
  arguments.}\footnote{For convenience, we use the dot (.) notation to
...
\\
\multicolumn{2}{l}{\textbf{Module} ($M$)} \\
$exports : \mathcal P (O)$      & \color{comment} // exported ops \\
$invokes : \mathcal P (O)$      & \color{comment} // invoked ops \\
$stores : N \pfun D$            & \color{comment} // stored data \\
$cons : O \pfun \mathcal P (F)$ & \color{comment} // constraints \ 
\end{tabular}
\end{minipage}
\quad
\begin{minipage}[t]{0.45\linewidth}
...
$args : N \pfun D$       & \color{comment} // arguments \\
\\
\multicolumn{2}{l}{\textbf{Data} ($D$)} \\
$fields : N \pfun D$     & \color{comment} // data fields \\
\\
\multicolumn{2}{l}{\textbf{View} ($V$)} \\
$mods : \mathcal P (M)$  & \color{comment} // modules \\
...
named \textit{arguments}\fix{, which the invoker uses to transmit data
  to its exporter}{; a module invoking an operation transmits data to
  the module exporting that operation by setting the operation's
