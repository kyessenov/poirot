...
they reach the limit.  \aleks{At the same time, for optimal user
  experience, new users should not be required to create an account to
  start reading the news articles.} One desirable property of this
...
acting like\comment{aleks}{as?} \textsf{ReplayAttacker}, extracts a
...
\comment{aleks}{CHECK: can we say the following?}
...
\aleks{It is important to note that, to merge her initial model with
  ...
  \textsf{ReplayAttacker}.  In contrast, Alice only had to specify her
...
\comment{aleks}{General comments: (1) renaming suggestions: GetPage
